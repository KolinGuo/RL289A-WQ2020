...
if __name__ == '__main__':
    ...
    args.add_argument("--num_room_states", type=int, default=7, help="Number of room states for one-hot encoding")
    ...
    for i in range(0, 60):
        grid = np.random.choice([False, True], size=(args.grid_height, args.grid_width, args.num_room_states))
    ...
    assert states.shape == (args.batch_size, args.grid_height, args.grid_width, args.num_room_states, args.grids_per_state), 'states shape error'
    assert actions.shape == (args.batch_size,), 'actions shape error'
    assert rewards.shape == (args.batch_size,), 'rewards shape error'
    assert next_states.shape == (args.batch_size, args.grid_height, args.grid_width, args.num_room_states, args.grids_per_state), 'next_states shape error'
